...
--VÍ DỤ: City IN('London','Berlin','NewYork') --thya bằng OR OR OR
...
SELECT * FROM Products WHERE CategoryID BETWEEN 1 AND 8 
...
SELECT * FROM Orders WHERE EmployeeID IN (5,6,7,9) --224 KO ĐC ĐIỂM
...
SELECT * FROM Categories WHERE CategoryID IN (SELECT CategoryID FROM Products WHERE SupplierID IN (SELECT SupplierID FROM Suppliers WHERE Country = 'USA'))
...
-- Liệt kê các đơn hàng cảu khách hàng đến từ Berlin, london, Madrid
SELECT * FROM [Order Details] WHERE OrderID IN (SELECT OrderID FROM Orders WHERE CustomerID IN (SELECT CustomerID FROM Customers WHERE City IN ('Berlin','London','Madrid')))
SELECT * FROM Orders
